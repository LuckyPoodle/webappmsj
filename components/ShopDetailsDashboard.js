...
const ShopDetailsDashboard = ({ shopData }) => {
  const {
    ...
    if (field == 'address') {
      try {
        ...
        const { data } = await axiosAuth.post(
          `/update-shop/${shopData[0]._id}`,
          { body: { values: addressValues } }
        )
        console.log(data)
      ...
      try {
        const { data } = await axiosAuth.post(
          `/update-shop/${shopData[0]._id}`,
          { body: { values: { shopContactNumber: shopContactNumber } } }
        )
        console.log(data)
        notify('Updated. Please refresh page', true)
        setOpen(false)
      ...
      try {
        const { data } = await axiosAuth.post(
          `/update-shop/${shopData[0]._id}`,
          { body: { values: { shopContactEmail: shopContactEmail } } }
        )
        console.log(data)
        notify('Updated. Please refresh page', true)
        setOpen(false)
      ...
      try {
        const { data } = await axiosAuth.post(
          `/update-shop/${shopData[0]._id}`,
          { body: { values: { bankAccountNumber: bankAccountNumber } } }
        )
        console.log(data)
        notify('Updated. Please refresh page', true)
        setOpen(false)
      ...
      try {
        const { data } = await axiosAuth.post(
          `/update-shop/${shopData[0]._id}`,
          { body: { values: { deliveryPickUpOption: deliveryPickUpOption } } }
        )
        console.log(data)
        notify('Updated. Please refresh page', true)
        setOpen(false)
      ...
      try {
        const { data } = await axiosAuth.post(
          `/update-shop/${shopData[0]._id}`,
          { body: { values: { deliveryPickUpInfo: deliveryPickUpInfo } } }
        )
        console.log(data)
        notify('Updated. Please refresh page', true)
        setOpen(false)
      ...
      try {
        const { data } = await axiosAuth.post(
          `/update-shop/${shopData[0]._id}`,
          { body: { values: { deliveryFee: deliveryFee } } }
        )
        console.log(data)
        notify('Updated. Please refresh page', true)
        setOpen(false)
      ...
      try {
        const { data } = await axiosAuth.post(
          `/update-shop/${shopData[0]._id}`,
          { body: { values: { shopTitle: shopTitle } } }
        )
        console.log(data)
        notify('Updated. Please refresh page', true)
        setOpen(false)
      ...
      try {
        const { data } = await axiosAuth.post(
          `/update-shop/${shopData[0]._id}`,
          { body: { values: { siteLinkOne: siteLinkOne } } }
        )
        console.log(data)
        notify('Updated. Please refresh page', true)
        setOpen(false)
      ...
      try {
        const { data } = await axiosAuth.post(
          `/update-shop/${shopData[0]._id}`,
          { body: { values: { siteLinkTwo: siteLinkTwo } } }
        )
        console.log(data)
        notify('Updated. Please refresh page', true)
        setOpen(false)
      ...
      try {
        const { data } = await axiosAuth.post(
          `/update-shop/${shopData[0]._id}`,
          { body: { values: { siteLinkThree: siteLinkThree } } }
        )
        console.log(data)
        notify('Updated. Please refresh page', true)
        setOpen(false)
      ...
      try {
        const { data } = await axiosAuth.post(
          `/update-shop/${shopData[0]._id}`,
          { body: { values: { siteLinkFour: siteLinkFour } } }
        )
        console.log(data)
        notify('Updated. Please refresh page', true)
        setOpen(false)
  ...
  return (
    <div className='h-screen overflow-y-auto'>
      <div className=' px-4 py-5 sm:px-6'>
        ...
        <br />
        <button
          onClick={toggleViewProductsDetails}
          type='button'
          class='text-white p-2 bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800'
        >
          View/Edit Products
        </button>
      </div>
        ...
        <dl>
          <div className='bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6'>
            ...
            <div className='row'>
              <dd className='mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2'>
                {shopData[0].shopTitle}
              </dd>
              <button onClick={() => handleEdit('shopTitle')}>edit</button>{' '}
            </div>
          </div>
          ...
          <div className='bg-gray-50  px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6'>
            <dt className='text-sm font-medium text-gray-500'>
              Shop Delivery/Pick up option
            </dt>
            <div className='row'>
              <dd className='mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2'>
                {shopData[0].deliveryPickUpOption}
              </dd>
              <button onClick={() => handleEdit('deliveryPickUpOption')}>
                edit
              </button>{' '}
            </div>
          </div>
          ...
          <div className='bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6'>
            ...
            <div className='row'>
              <dd className='mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2'>
                {shopData[0].deliveryFee}
              </dd>
              <button onClick={() => handleEdit('deliveryFee')}>edit</button>{' '}
            </div>
          </div>

          <div className='bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6'>
            <dt className='text-sm font-medium text-gray-500'>
              Shop Delivery/Pick up Info
            </dt>
            <div className='row'>
              <dd className='mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2'>
                {shopData[0].deliveryPickUpInfo}
              </dd>
              <button onClick={() => handleEdit('deliveryPickUpInfo')}>
                edit
              </button>{' '}
            </div>
          </div>
          ...
          <div className='bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6'>
            ...
            <div className='row'>
              <dd className='mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2'>
                {shopData[0].bankAccountNumber}
              </dd>
              <button onClick={() => handleEdit('bankAccountNumber')}>
                edit
              </button>{' '}
            </div>
          </div>
          <div className='bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6'>
            ...
            <div className='row'>
              ...
              <button onClick={() => handleEdit('bankName')}>edit</button>{' '}
            </div>
          </div>
          <div className='bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6'>
            <dt className='text-sm font-medium text-gray-500'>
            ...
            <div className='row'>
              <dd className='mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2'>
                {shopData[0].shopContactEmail}
              </dd>
              <button onClick={() => handleEdit('shopContactEmail')}>
                edit
              </button>{' '}
            </div>
          </div>
          ...
          <div className='bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6'>
            ...
            <div className='row'>
              <dd className='mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2'>
                {shopData[0].shopContactNumber}
              </dd>
              <button onClick={() => handleEdit('shopContactNumber')}>
                edit
              </button>{' '}
            </div>
          </div>
          <div className='bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6'>
            ...
            <div className='row'>
              <dd className='mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2'>
                {shopData[0].siteLinkOne}
              </dd>
              <button onClick={() => handleEdit('siteLinkOne')}>edit</button>{' '}
            </div>
          </div>
          ...
          <div className='bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6'>
            ...
            <div className='row'>
              <dd className='mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2'>
                {shopData[0].siteLinkTwo}
              </dd>
              <button onClick={() => handleEdit('siteLinkTwo')}>edit</button>{' '}
            </div>
          </div>

          <div className='bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6'>
            <dt className='text-sm font-medium text-gray-500'>
              Shop Site Link Three
            </dt>
            <div className='row'>
              <dd className='mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2'>
                {shopData[0].siteLinkThree}
              </dd>
              <button onClick={() => handleEdit('siteLinkThree')}>edit</button>{' '}
            </div>
          </div>
          ...
          <div className='bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6'>
            ...
            <div className='row'>
              <dd className='mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2'>
                {shopData[0].siteLinkFour}
              </dd>
              <button onClick={() => handleEdit('siteLinkFour')}>edit</button>{' '}
            </div>
          </div>
                    ...
                    <div className='sm:flex sm:items-start'>
                      ...
                      <div className='mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left'>
                        ...
                        <div className='mt-2'>
                          ...
                          ) : fieldToEdit == 'deliveryPickUpInfo' ? (
                            <>
                              <div>
                                <label
                                  htmlFor='about'
