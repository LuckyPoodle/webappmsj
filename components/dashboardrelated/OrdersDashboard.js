...
const OrdersDashboard = ({ shopId }) => {
  ...
  async function handleOrderStatusChange(e, shopOrderId) {
    setLoading(true)
    //update UI
    let indexoforder = shopOrders.findIndex(
      (order) => order._id === shopOrderId
    )
    let newshopOrders = shopOrders
    newshopOrders[indexoforder].orderStatus = e.target.value

    const { data } = await axiosAuth.post(
      `/update-order-status/${shopOrderId}`,
      {
  ...
  return (
    <div className='h-full'>
      ...
      >
        {orderPurchasedView ? 'View Shop Orders' : 'View Purchased Orders'}
      </button>
      <div className='col'>
        {orderPurchasedView ? (
          <>
                          ...
                          <div className='relative p-5 shadow-md '>
                            <div className='flex flex-col items-center p-5'>
                              ...
                              {order.shopProductsInOrder.map((pdt) => {
                                return (
                                  <span>
                                    {pdt.name} ${pdt.price} --- {pdt.shopName}
                                  </span>
                                )
                              })}

                              <button className='bg-indigo-500 p-2 text-white w-1/4 rounded text-center italic'>
                                {' '}
                                <Link
                                  href={`/${order.shopProductsInOrder[0].shopSlug}`}
                                >
                                  <a className=''>View Shop</a>
                                </Link>
                              </button>
                            </div>
                          </div>
              ...
              <div className=''>
                <div class='flex flex-wrap h-full'>
                  {shopOrders.length > 0 ? (
                          ...
                          <div class=' flex-initial pt-10 w-full h-full '>
                            <div className='relative p-5 shadow-md '>
                              ...
                              {order.paymentMethod === 'Credit' ? (
                                <button className='bg-indigo-500 p-2 m-2 text-white w-50 h-50 rounded text-center italic'>
                                  Card Payment: Submit Refund Request
                                </button>
                              ) : (
                                <></>
                              )}
                    ...
                    <div className='p-10'>
                      <span>You have not sold anything yet!</span>
                    </div>
                  )}
                </div>
              </div>
