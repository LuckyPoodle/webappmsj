...
const ProductsDetailsDashboard = ({ shopData }) => {
  ...
  const fetchShopProducts = async () => {
    try {
      ...
      const { data } = await axiosAuth.get(
        `/get-shop-products/${shopData[0].slug}`,
        {}
      )
      console.log(data)
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      productValues.category.trim() == '' ||
      productValues.name.trim() == '' ||
      productValues.description.trim() == '' ||
    ...
    if (
      productValues.stockAvailable < 0 ||
      productValues.price < 0 ||
      productValues.deliveryAvailable < 0
    ) {
    ...
    if (!isEditing) {
      try {
        ...
        const { data } = await axiosAuth.post(
          `/create-product/${shopData[0]._id}`,
          { body: { values: productValues } }
        )
      ...
      try {
        ...
        productValues.longitude = shopData[0].longitude
        const { data } = await axiosAuth.post(
          `/update-product/${productValues.id}`,
          { body: { values: productValues } }
        )
  ...
  const handleDeleteProduct = async () => {
    ...
    try {
      const { result } = await axiosAuth.post(
        `/delete-product/${productValues.id}`
      )
    ...
    <div className='bg-white'>
      <div className='max-w-2xl mx-auto py-16 px-4 sm:py-24 sm:px-6 lg:max-w-7xl lg:px-8'>
        <button
          ...
          type='button'
          className='text-white p-2 bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800'
        >
          View/Edit Shop
        </button>
        <button
          ...
          type='button'
          className='text-white p-2 bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-green-600 dark:hover:bg-green-700 focus:outline-none dark:focus:ring-green-800'
        >
          Add product
        </button>
                ...
                <div className='col'>
                    ...
                    <div className='w-full min-h-80 bg-gray-200 aspect-w-1 aspect-h-1 rounded-md overflow-hidden group-hover:opacity-75 lg:h-80 lg:aspect-none'>
                      <img
                        src={
                          product.mainImage
                            ? product.mainImage
                            : 'https://media.publit.io/file/Untitled-4-h.jpeg'
                        }
                  ...
                  <div className='flex justify-between'>
                    ...
                    {product.outOfStock == false ? (
                      <button
                        className='text-black'
                        onClick={() => handleProductSetOutOfStock(product._id)}
                      >
                        Set Out-Of-Stock
                      </button>
                    ) : (
                      <button
                        className='text-black'
                        onClick={() => handleProductSetAvailable(product._id)}
                      >
                        Set Available
                      </button>
                    )}
                    <button
                      className=''
                      onClick={() => handleConfirmationDialog(product._id)}
                    >
                      {' '}
                      <TrashIcon
      ...
      <ConfirmationDialog
        ...
        handleCancel={handleConfirmationDialog}
      ...
      <Transition.Root show={open} as={Fragment}>
        <Dialog
          as='div'
          className='fixed pt-10  z-60 inset-0 overflow-y-auto'
          initialFocus={cancelButtonRef}
          onClose={exitEditProduct}
        >
                ...
                <div className='bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4'>
                  <div className='sm:flex sm:items-start'>
                    ...
                    <div className='mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left'>
                      <Dialog.Title
                        as='h3'
                        className='text-lg pb-10 leading-6 font-medium text-gray-900'
                      >
                        Add/Edit product
                      </Dialog.Title>
                    </div>
                  ...
                  <EditProductForm
                    ...
                    exitEditProduct={exitEditProduct}
                    handleSubmit={handleSubmit}
                    handleMainImageRemove={handleMainImageRemove}
                    uploadMainImageLoading={uploadMainImageLoading}
                    loading={loading}
                    images={images}
                    setImages={setImages}
