              ...
              <div className='xl:w-3/5 flex flex-col sm:flex-row xl:flex-col justify-center items-center bg-white py-3 sm:py-0 xl:py-10 px-10 xl:w-full'>
                <div className='flex flex-col justify-start items-start w-full space-y-4'>
                  {products.map((pdt) => (
                    <p
                      key={pdt}
                      className='text-xl md:text-2xl leading-normal text-gray-800'
                    >
                      {pdt.name} - ${pdt.price} -{' '}
                      {pdt.shipping == 'false' ? 'Pick Up' : 'Delivery'}{' '}
                    </p>
                  ))}

                  <p>Delivery Charges : ${deliveryCost}</p>
                  <p>Amount Due: </p>
                  <p className='text-base font-semibold leading-none text-2xl p-1 text-black'>
                    ${total}
                  </p>
                  <div>
                    <br />
                    ...
                    <div className='flex flex-row'>
                      ...
                      <p className='pt-3'>Save Info for future checkout</p>
                    </div>
                  </div>
                </div>
              </div>
                  ...
                  <button className='border border-transparent  bg-gray-900 text-white flex flex-row justify-center items-center space-x-2 py-4 rounded w-full'>
                    <div>
                      <p className='text-base leading-4'>
                        Cash Payment Not Available For Delivery Order
                      </p>
                    </div>
                  </button>
                ) : (
                  <button
                    disabled={contactNumber.length == 0}
                    onClick={createCashOrder}
                    className='border border-transparent hover:border-gray-300 bg-gray-900 hover:bg-white text-white hover:text-gray-900 flex flex-row justify-center items-center space-x-2 py-4 rounded w-full'
                  >
                    <div>
                      <p className='text-base leading-4'>
                        Pay Cash On Pick Up{' '}
