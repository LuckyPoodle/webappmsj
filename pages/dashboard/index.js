...
function Dashboard({ shopData, statistics }) {
  ...
  const [totalRevenuePendingCompletion, setTotalRevenuePendingCompletion] =
    useState(0)
  const [totalOrdersPendingCompletion, setTotalOrdersPendingCompletion] =
    useState(0)
  ...
  const getAccountStatistics = () => {
    if (statistics !== []) {
      ...
      setTotalCompletedRevenue(
        statistics.completedRevenue.totalCompletedRevenue
      )
      ...
      let revenuepending =
        ...
        statistics.completedRevenue.totalCompletedRevenue
      let orderspending =
        statistics.totalCount - statistics.completedRevenue.totalCompletedCount
        ...
        <main>
          <div className='max-w-7xl mx-auto py-6 sm:px-6 lg:px-8 min-h-screen'>
            <div className='px-4 py-6 sm:px-0'>
              ...
              {current == '#overview' ? (
                <DashboardOverview
                  purchasedProducts={shopData[0].owner.purchasedProducts}
                  orders={shopData[0].owner.orders}
                  shopTitle={shopData[0].shopTitle}
                  totalOrdersPendingCompletion={totalOrdersPendingCompletion}
                  monthlyRevenueCount={monthlyRevenueCount}
                  monthlyOrderCount={monthlyOrderCount}
                  total={totalRevenue}
                  totalCompletedOrders={totalCompletedOrders}
                  totalOrders={totalOrders}
                  totalCompletedRevenue={totalCompletedRevenue}
                  totalRevenuePendingCompletion={totalRevenuePendingCompletion}
                />
              ) : current == '#shop' ? (
                <ShopDashboardOverview shopData={shopData} />
              ) : current == '#orders' ? (
                <OrdersDashboard
                  shopId={shopData[0]._id}
                  purchasedOrders={shopData[0].owner.orders}
                />
              ) : (
                <h1>Other</h1>
              )}
...
export const getServerSideProps = async ({ req, res }) => {
  try {
    ...
    const { data } = await axios.get(
      `${process.env.api}/get-shop-details-for-owner`,
      {
        headers: {
    ...
    if (data[0]._id) {
      const statistics = await axios.get(
        `${process.env.api}/get-statistics/${data[0]._id}`,
        {
          headers: {
            token: req.cookies.token,
          },
